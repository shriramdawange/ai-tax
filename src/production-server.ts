...
const tenantRouter=(factory:(pool:Pool,org:string)=>express.Router())=>[authRequired(pool),(req:Request,res:Response,next:NextFunction)=>factory(pool,req.auth!.organizationId)(req,res,next)];
...
app.use((err:any,_req:Request,res:Response,_next:NextFunction)=>{console.error(err);if(res.headersSent)return;res.status(500).json({error:process.env.NODE_ENV==='production'?'Internal server error':String(err?.message??err)});});
