...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL ?? 'postgres://ai_tax:ai_tax_dev@localhost:5432/ai_tax'
});
...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    res.json({ ok: true, database: r.rows[0].ok === 1, service: 'ai-tax-api' });
  } catch {
...
app.get('/api/dashboard', async (_req, res) => {
  ...
  const totals = tb.reduce((acc: {debit:number;credit:number}, row) => ({debit:acc.debit+Number(row.debit), credit:acc.credit+Number(row.credit)}), {debit:0,credit:0});
  res.json({ organization: org.rows[0], trialBalance: tb, totals, exceptions: exceptions.rows });
});

app.get('/api/accounts', async (_req,res) => {
  const r = await pool.query('SELECT id,code,name,type,parent_id FROM accounts WHERE organization_id=$1 ORDER BY code',[ORG]);
  res.json(r.rows);
});

app.get('/api/parties', async (_req,res) => {
  const r = await pool.query('SELECT id,name,kind,gstin,pan,state_code FROM parties WHERE organization_id=$1 ORDER BY name',[ORG]);
  res.json(r.rows);
});
...
app.post('/api/journals', async (req,res) => {
  ...
  try {
    const id = await postJournal(pool,{organizationId:ORG,financialPeriodId:PERIOD,...parsed.data});
    res.status(201).json({id,status:'POSTED'});
  } catch (e) {
    res.status(400).json({error:e instanceof Error ? e.message : 'Unable to post journal'});
  }
});

app.get('/api/ledger/:accountId', async (req,res) => {
  const r = await pool.query(`SELECT je.entry_date,je.voucher_number,je.voucher_type,je.narration,jl.description,jl.debit,jl.credit
    FROM journal_lines jl JOIN journal_entries je ON je.id=jl.journal_id
    WHERE je.organization_id=$1 AND jl.account_id=$2 AND je.status='POSTED'
    ORDER BY je.entry_date,je.created_at`,[ORG,req.params.accountId]);
  let balance = 0;
  const rows = r.rows.map(row => { balance += Number(row.debit)-Number(row.credit); return {...row,balance:balance.toFixed(2)}; });
  res.json(rows);
});

app.get('/api/compliance/summary', async (_req,res) => {
  const [gst,t ds] = await Promise.all([
    pool.query(`SELECT COALESCE(SUM(CASE WHEN direction='OUTPUT' THEN cgst+sgst+igst+cess ELSE 0 END),0) output_tax,
      COALESCE(SUM(CASE WHEN direction='INPUT' THEN cgst+sgst+igst+cess ELSE 0 END),0) input_tax
      FROM gst_transactions WHERE organization_id=$1`,[ORG]),
